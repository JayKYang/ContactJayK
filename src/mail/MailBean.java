...
public class MailBean {
	...
	public void codeGenerator() {
		String codenum[] = new String[4];
		
		for(int i=0; i<codenum.length; i++) {
			codenum[i] = Integer.toString((int)(Math.random()*9)+1);
		}
		setCode(codenum.toString());
	}
	// 생성자
	public MailBean(String recipient) {
		// 인증에 사용할 기본값 설정
		sender = "dumbyura@naver.com"; // 본인 naver email
		passwd = "18didwlsrb>"; // 본인 naver pw
		this.recipient = recipient; // 수신 email
		this.title = "JayK's Homepage Verification Code";
		codeGenerator();
		contents="인증번호 ["+getCode()+"]";
		err_msg="오류";
	}
